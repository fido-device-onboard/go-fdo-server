# FIDO Device Onboarding User Guide
...
The `go-fdo-client` application is a Go implementation of a device client. It is available from the [Go FDO Client project](https://github.com/fido-device-onboard/go-fdo-client).
...
**Rendezvous server**  
In cases where the device’s ownership is not known at the time of manufacture, a discovery service is needed so the device can identify its owner at the start of onboarding. The Rendezvous server provides this discovery service. The device client contacts the Rendezvous server at the start of the onboarding process to retrieve the Owner server’s network address(es).
...
The `go-fdo-server` application is a Go implementation of all three server components. Each instance of `go-fdo-server` functions as one of the above servers. Its role is selected at startup via a subcommand (e.g. `go-fdo-server manufacturing`). It is available from the [Go FDO Server project](https://github.com/fido-device-onboard/go-fdo-server).
...
2. On-site Onboarding (FDO Transfer Ownership Protocols: TO0, TO1, TO2)
   1. Prior to first boot of the device, the Ownership Voucher is uploaded to the Owner server by the server administrator.
   2. The Owner server reads the Rendezvous server’s network address from the Ownership Voucher and initiates the TO0 registration protocol.
   ...
   6. On the first boot, the device client retrieves the Rendezvous server network address from its on-board device credentials and contacts the Rendezvous server.
   7. The Rendezvous server looks up the device by its GUID and returns the Owner server’s network address(es) to the device client.
...
All FDO servers support network access via HTTP. Servers can be configured to enable HTTPS for security purposes. In this case, additional TLS certificates for server authentication need to be provided. These certificates are separate from the certificates required by FDO. Refer to the [Certificate Setup Guide](certificates.md) and the [Server Configuration Reference](server-config.md) for additional information.

#### Database Support

All FDO servers require a database in order to persist state across restarts. The FDO servers can be configured to use either SQLite or PostgreSQL as the database implementation. Refer to the [Server Configuration Reference](server-config.md) for additional information.
