...
```bash
# dnf install ./rpmbuild/rpms/noarch/go-fdo-server-manufacturer-*.noarch.rpm ./rpmbuild/rpms/x86_64/go-fdo-server-*.x86_64.rpm
```

The packages install a number of files, create the `go-fdo-server` group and `go-fdo-server-manufacturer` user, and install a `systemd` service for running the Manufacturing server. Check if the Manufacturing server's `systemd` unit file is available:
...
```bash
# firewall-cmd --add-port=8038/tcp --permanent --reload
```
...
```bash
$ curl http://manufacturer.example.com:8038/health
...
```bash
# dnf install ./rpmbuild/rpms/noarch/go-fdo-server-rendezvous-*.noarch.rpm ./rpmbuild/rpms/x86_64/go-fdo-server-*.x86_64.rpm
```

The packages install a number of files, create the `go-fdo-server` group and `go-fdo-server-rendezvous` user, and install a `systemd` service for running the Rendezvous server. Check if the Rendezvous server's `systemd` unit file is available:
...
```bash
# firewall-cmd --add-port=8041/tcp --permanent --reload
```
...
```bash
$ curl http://rendezvous.example.com:8041/health
{"message":"the service is up and running","status":"OK","version":"1.0.0"}
```
...
```bash
# dnf install ./rpmbuild/rpms/noarch/go-fdo-server-owner-*.noarch.rpm ./rpmbuild/rpms/x86_64/go-fdo-server-*.x86_64.rpm
```

The packages install a number of files, create the `go-fdo-server` group and `go-fdo-server-owner` user, and install a `systemd` service for running the Owner server. Check if the Owner server's `systemd` unit file is available:
...
```bash
# firewall-cmd --add-port=8043/tcp --permanent --reload
```
...
```bash
$ curl http://owner.example.com:8043/health
{"message":"the service is up and running","status":"OK","version":"1.0.0"}
```
