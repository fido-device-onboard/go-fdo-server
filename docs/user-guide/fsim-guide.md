...
The `fdo.command` FSIM can be used by the Owner server to execute programs on the device. These programs are executed by the device client and must be present on the device prior to issuing the `fdo.command`. The `fdo.command` supports passing arguments to the program, and redirecting `stdout` and `stderr` back to the Owner server's logs. The `fdo.command` supports the inlining of shell scripts directly into the configuration file.
