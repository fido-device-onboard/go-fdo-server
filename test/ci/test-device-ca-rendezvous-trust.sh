...
run_test() {
  ...
  log_info "List the rendezvous Device CA certificates"
  fingerprint=$(list_device_ca_certs ${rendezvous_url} | jq -r -M '.certs[0].fingerprint')
  ...
  log_info "Running FIDO Device Onboard (must fail)"
