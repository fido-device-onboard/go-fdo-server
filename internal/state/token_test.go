...
func TestCleanupExpiredSessions(t *testing.T) {
	...
	_, err = tokenService.NewToken(ctx, protocol.TO1Protocol)
	if err != nil {
	...
	// Verify we can still use the fresh token
	ctx = tokenService.TokenContext(ctx, token)
	_, ok := tokenService.TokenFromContext(ctx)
	if !ok {
		t.Fatal("Failed to retrieve token from context")
	}
}
