...
//
// All database operations are wrapped in a single transaction so that either
// all certificates are imported or none are (on error).
func (s *TrustedDeviceCACertsState) ImportDeviceCACertificates(ctx context.Context, pemData string) (*CertificateImportStats, error) {
	...
	// Parse all PEM blocks and certificates upfront (no DB access needed)
	...
	for {
		...
		parsed = append(parsed, parsedCert{cert: cert, position: position})
		remaining = rest
	}

	// Import all valid certificates inside a single transaction
	if len(parsed) > 0 {
		err := s.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			for _, p := range parsed {
				hash := sha256.Sum256(p.cert.Raw)
				...
				if existingCount > 0 {
					stats.Skipped++
					stats.Messages = append(stats.Messages, fmt.Sprintf("the certificate at position %d with subject '%s' was skipped because it already exists", p.position, p.cert.Subject.String()))
					continue
				}
				...
				dbCert := DeviceCACertificate{
					...
					PEM:         string(certPEM),
					Subject:     p.cert.Subject.String(),
					Issuer:      p.cert.Issuer.String(),
				...
				stats.Imported++
				stats.Messages = append(stats.Messages, fmt.Sprintf("the certificate at position %d with subject '%s' was imported successfully", p.position, p.cert.Subject.String()))
