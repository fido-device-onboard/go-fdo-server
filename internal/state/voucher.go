...
func (s *VoucherPersistentState) ExtendVoucher(
	...
	err := s.DB.Transaction(func(tx *gorm.DB) error {
		// Create transactional state wrapper
		txState := &VoucherPersistentState{
			Token: s.Token,
			DB:    tx,
		}

		// Get current voucher
		voucher, err := txState.Voucher(ctx, guid)
		if err != nil {
			return err
		}

		// Extend voucher using fdo package
		// This cryptographically signs the voucher with the current owner's key
		// and adds a new entry transferring ownership to the next owner
		// We need to type assert the crypto.PublicKey interface to a concrete type
		// that satisfies protocol.PublicKeyOrChain constraint
		...
		case *rsa.PublicKey:
			extended, err = fdo.ExtendVoucher(voucher, currentOwnerKey, key, nil)
		default:
			return fmt.Errorf("unsupported public key type: %T", nextOwnerKey)
		}
		if err != nil {
		...
		return txState.ReplaceVoucher(ctx, guid, extended)
	})
