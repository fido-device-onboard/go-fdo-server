...
func FetchVoucher(filters map[string]interface{}) (Voucher, error) {
	var voucher Voucher
	if len(filters) == 0 {
		return voucher, fmt.Errorf("no filters provided")
	}

	query := "SELECT guid, device_info, cbor, created_at, updated_at FROM owner_vouchers WHERE 1=1"
	args := make([]interface{}, 0, 2)

	if v, ok := filters["guid"]; ok {
		b, ok := v.([]byte)
		if !ok {
			return voucher, fmt.Errorf("invalid type for guid filter; want []byte")
		}
		query += " AND guid = ?"
		args = append(args, b)
	}
	if v, ok := filters["device_info"]; ok {
		s, ok := v.(string)
		if !ok {
			return voucher, fmt.Errorf("invalid type for device_info filter; want string")
		}
		query += " AND device_info = ?"
		args = append(args, s)
	}

	// Fetch up to two rows to detect duplicates
	...
	if !rows.Next() {
		return voucher, sql.ErrNoRows
	}
	var createdInt, updatedInt int64
	if err := rows.Scan(&voucher.GUID, &voucher.DeviceInfo, &voucher.CBOR, &createdInt, &updatedInt); err != nil {
		return voucher, err
	}
	voucher.CreatedAt = time.UnixMicro(createdInt)
	voucher.UpdatedAt = time.UnixMicro(updatedInt)

	// If there is a second row, error out due to ambiguity
	if rows.Next() {
		return Voucher{}, fmt.Errorf("multiple vouchers matched filters")
	}

	return voucher, nil
}

// QueryVouchers returns all owner vouchers matching optional filters (without CBOR), ordered by updated_at DESC.
func QueryVouchers(filters map[string]interface{}) ([]Voucher, error) {
	query := "SELECT guid, device_info, created_at, updated_at FROM owner_vouchers WHERE 1=1"
	args := make([]interface{}, 0, 2)
	if v, ok := filters["guid"]; ok {
		b, ok := v.([]byte)
		if !ok {
...
// ListVouchers returns all owner vouchers without CBOR payload, ordered by most recently updated.
func ListVouchers() ([]Voucher, error) {
	rows, err := db.Query("SELECT guid, device_info, created_at, updated_at FROM owner_vouchers ORDER BY updated_at DESC")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
