...
// MfgVoucher stores manufacturer vouchers
type MfgVoucher struct {
	GUID       []byte    `gorm:"primaryKey"`
	DeviceInfo string    `gorm:"type:text;not null"`
	CBOR       []byte    `gorm:"not null"`
	CreatedAt  time.Time `gorm:"autoCreateTime:milli"`
	UpdatedAt  time.Time `gorm:"autoUpdateTime:milli"`
}

// TableName specifies the table name for MfgVoucher model
func (MfgVoucher) TableName() string {
	return "mfg_vouchers"
}
