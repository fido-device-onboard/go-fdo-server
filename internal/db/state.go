...
func (s *State) DeviceCertChain(ctx context.Context) ([]*x509.Certificate, error) {
	...
	// Parse certificates using ASN.1 to determine certificate boundaries
	var chain []*x509.Certificate
	remaining := deviceInfo.X509Chain
	for len(remaining) > 0 {
		// Use ASN.1 unmarshaling to find the boundary of this certificate
		...
		// Extract just this certificate's bytes
		certBytes := remaining[:len(remaining)-len(rest)]
