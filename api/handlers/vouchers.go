...
func InsertVoucherHandler(rvInfo *[][]protocol.RvInstruction) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for block, rest := pem.Decode(body); block != nil; block, rest = pem.Decode(rest) {
			...
			newRvInfo, err := rvinfo.GetRvInfoFromVoucher(block.Bytes)
			if err != nil {
				slog.Debug("Unable to get rvinfo from voucher", "err", err.Error())
				http.Error(w, "Unable to get rvinfo from voucher", http.StatusInternalServerError)
				return
			}
			*rvInfo = newRvInfo
		}
