...
func (s *Server) verifyVoucherOwnership(ov *fdo.Voucher) error {
	...
	eq, ok := expectedPubKey.(interface{ Equal(crypto.PublicKey) bool })
	if !ok {
		return fmt.Errorf("voucher owner public key type %T does not support equality comparison", expectedPubKey)
	}
	if !slices.ContainsFunc(s.OwnerPKeys, eq.Equal) {
