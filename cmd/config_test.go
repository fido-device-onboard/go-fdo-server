...
`
	...
	if manufacturingKey != "/path/to/mfg.key" {
		t.Fatalf("manufacturingKey=%q", manufacturingKey)
	}
	if deviceCACert != "/path/to/device.ca" {
		t.Fatalf("deviceCACert=%q", deviceCACert)
	}
	if deviceCAKey != "/path/to/device.key" {
		t.Fatalf("deviceCAKey=%q", deviceCAKey)
	}
	if ownerPKey != "/path/to/owner.crt" {
		t.Fatalf("ownerPKey=%q", ownerPKey)
	}
}
